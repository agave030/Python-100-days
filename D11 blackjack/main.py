...
# Hint 1: Go to this website and try out the Blackjack game(done)
#   http://blackjack-final.appbrewery.repl.run

# Hint 2: Read this breakdown of program requirements(done)
#   http://listmoz.com/view/6h34DJpvJBFVRlZfJvxF
# Then try to create your own flowchart for the program.

# Hint 3: Download and read this flow chart I've created(done)
...
# my first version without hints
...
def deal_card():
    ...
    return choice(cards)
def ace_check(cards):
    """if ace appear in the cards and the sum of cards is bigger than 21, the point of ace will be replaced as 1"""
    if 11 in cards and sum(cards) > 21:
        cards.remove(11)
        cards.append(1)
def final_print(player_cards, computer_cards):
    """print final result out"""
    print(f"Your final hand: {player_cards}, final score: {sum(player_cards)}")
    print(f"Computer's final hand: {computer_cards}, final score: {sum(computer_cards)}")
def play_game():
    """the main game function"""
    # deal card twice and reveal the situation now
    player_cards = []
    computer_cards = []
    for _ in range(2):
    ...
    # this if statement is for computer's blackjack check.
    # if the computer has blackjack, no matter the player has blackjack, computer still win
    if sum(computer_cards) == 21:
        final_print(player_cards, computer_cards)
        print("You lose. Opponent has Blackjack.")

    # now it's player turn. the player can choose to draw a card or not
    # use the variable keep_drawing as a flag for while loop
    else:
        keep_drawing = True
        while keep_drawing:
